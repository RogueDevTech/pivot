...
const Boundaries: React.FC = () => {
  ...
  return (
    <section className="flex flex-col gap-[32px]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 justify-between lg:w-[729px] w-full gap-6">
        {researchWithImages.map((item) => (
          <Link
            key={item.slug}
            href={`/research/${item.slug}`}
            className="flex flex-col justify-between gap-[16px] w-full lg:w-[360px] group cursor-pointer"
          >
            <div className="relative lg:w-[357px] h-[239px] w-full overflow-hidden">
              <Image
                src={item.image}
                fill
                alt={item.title}
